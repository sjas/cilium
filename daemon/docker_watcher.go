...
func (d *Daemon) handleCreateContainer(id string, retry bool) {
	...
	for try := 1; try <= maxTries; try++ {
		...
		if ok {
			...
			if orchLabelsModified = updateOrchLabels(container, lbls); !orchLabelsModified {
				log.Debugf("No changes to orch labels, ignoring")
			}
		} else {
			container = types.NewContainer(dockerContainer, lbls)
			container.Mutex.Lock()
		}

		// It's mandatory to update the container in its label otherwise
		// the label will be considered unused.
		identity, newHash, err := d.updateContainerIdentity(container.ID, container.LabelsHash, &container.OpLabels)
		if err != nil {
			container.Mutex.Unlock()
			log.Warningf("unable to update identity of container %s: %s", id, err)
			return
		}
		container.LabelsHash = newHash
		cID := container.ID
		container.Mutex.Unlock()
		...
		d.containers[epDockerID] = container
		d.containersMU.Unlock()
